function postBox() {
    ...
    deletePost.forEach((deletePost, index) => {
        deletePost.onclick = () => {
            ...
            query.send("POST");
            let query2 = new QueryData("/notification/deleteAllNotificationOfStatus");
            query2.addParam("reference", `status=${deletePostUser.id}`);
            query2.send("POST");
    ...
    mainSend.forEach((mainSend, index) => {
        ...
        mainSend.addEventListener("click", () => {
            ...
            query.addEvent("load", function () {
                ...
                query3.addParam("statusid", mainSend.parentNode.parentNode.parentNode.parentNode.id);
                query3.addParam("emojiid", 7);
                query3.addParam("reference", `status=${mainSend.parentNode.parentNode.parentNode.parentNode.id}&comment=${commentId}`);
                query3.send("POST");
        ...
        deleteComment.forEach((comment) => {
            comment.onclick = () => {
                ...
                if (mainComment.className.includes("main-comment")) {
                    ...
                    let query3 = new QueryData("/notification/deleteNotification");
                    query3.addParam("emojiid", "7");
                    query3.addParam("reference", `status=${mainComment.parentNode.parentNode.parentNode.parentNode.id}&comment=${mainComment.id}`);
                    query3.send("POST");
                    mainComment.remove();
                }
                if (levelComment.className.includes("comment-level")) {
